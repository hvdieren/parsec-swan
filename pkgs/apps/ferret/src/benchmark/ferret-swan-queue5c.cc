...
int scan_dir (const char *dir, char *head, pushdep<load_data> * load_queue);//__attribute__((always_inline));
void file_helper (const char *file, pushdep<load_data>* load_queue)__attribute__((always_inline));
struct load_data* load_array[5000];
unsigned int array_size = -1;

int dir_helper (char *dir, char *head, pushdep<load_data> * load_queue)
{
	DIR *pd = NULL;
	...
	for (;;)
	{
		ent = readdir(pd);
		if (ent == NULL) break;
		if (scan_dir( ent->d_name, head, load_queue) != 0) return -1;
	}
	goto final;
...
void file_helper (const char *file, pushdep<load_data> * load_queue)
{
	int r;
	...
	load_queue->push( data );
}

int scan_dir (const char *dir, char *head, pushdep<load_data> * load_queue)
{
	struct stat st;
	int ret;
	/* test for . and .. */
	...
	if (S_ISREG(st.st_mode)) file_helper(path, load_queue);
	else if (S_ISDIR(st.st_mode))
	{
		strcat(head, "/");
		dir_helper(path, head + strlen(head), load_queue);
	}
...
/* ------ The Stages ------ */
void t_load (const char * dir, pushdep<load_data> load_queue)
{
	path[0] = 0;

	if (strcmp(dir, ".") == 0)
	{
	    dir_helper(".", path, &load_queue);
	}
	else
	{
		scan_dir(dir, path, &load_queue);
	}
}
...
void t_sxvrs(load_data ld, pushdep<rank_data> rank_queue) {
	load_data * load = &ld;
	seg_data seg;
	rank_data rank;
	extract_data x;
	...
	rank_queue.push( rank );
}

void t_sxvr(popdep<load_data> load_queue, pushdep<rank_data> rank_queue) {
	while( !load_queue.empty() ) {
		load_data ld = load_queue.pop();
		spawn(t_sxvrs, ld, rank_queue);
	}
	ssync();
}
...
void t_out_wrapper( popdep<rank_data> rank_queue )
{
	while( !rank_queue.empty() ) {
		rank_data rank = rank_queue.pop();
		leaf_call( t_out, &rank );
	}
}
...
*/
	...
	hyperqueue<rank_data> rank_queue;

	spawn(t_load, query_dir, (pushdep<load_data>)load_queue);
	spawn(t_sxvr, (popdep<load_data>)load_queue, (pushdep<rank_data>) rank_queue );
	
	spawn( t_out_wrapper, (popdep<rank_data>)rank_queue );
	ssync();
