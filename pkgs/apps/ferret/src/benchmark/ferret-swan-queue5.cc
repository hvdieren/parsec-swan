...
*/
	...
	while( !load_queue.empty() ) {
		spawn(t_seg_wrapper, (popdep<load_data>)load_queue, (outdep<seg_data>)segdataobj);
		spawn(t_extract_wrapper, (indep<seg_data>)segdataobj, (outdep<extract_data>) extractarg);
		spawn(t_vec_wrapper, (indep<extract_data>) extractarg, (outdep<vec_query_data>) vecarg);
		spawn(t_rank_wrapper, (indep<vec_query_data>) vecarg, (indep<extract_data>)extractarg, (outdep<rank_data>) rankarg);
		spawn(t_out_wrapper, (indep<rank_data>) rankarg, (inoutdep<int>)wait);
	}
	ssync();
