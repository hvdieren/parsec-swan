...
void ReorderSwan(struct thread_args * targs, indep<list<chunk_t*>> chunks_obj, int fd, int iteration, inoutdep<int> wait)
{
...
//stage 4 function
void CompressSwan(struct thread_args * targs, inoutdep<list<chunk_t *>> chunk_obj)
{
	struct thread_args *args = (struct thread_args *)targs;
...
//stage 3 function
void DeduplicateSwan(struct thread_args * targs, inoutdep<list<chunk_t*>> chunklist_obj, inoutdep<int> wait)
{
	struct thread_args *args = (struct thread_args *)targs;
...
void SwanPipeline(struct thread_args *args)
{
	...
	*pr_buff_seek_ptr = pr_buff_seek;
	...
	object_t<chunk_t*>	result_obj, chunk_obj, temp_obj;
	...
	while(1)
	{
		...
		if(!(read_done) || iteration == 0)
		{
			spawn(FragmentRefineSwan, args, 		(indep<chunk_t*>)result_obj, 
															...
															iteration);
			spawn(DeduplicateSwan, args, 	(inoutdep<list<chunk_t*>>)chunks, (inoutdep<int>) wait);
			spawn(CompressSwan, args, (inoutdep<list<chunk_t*>>) chunks);

			spawn(ReorderSwan, args, 	(indep<list<chunk_t*>>)chunks, fd, iteration, (inoutdep<int>) wait2);
		}
		else
			break;
	}
