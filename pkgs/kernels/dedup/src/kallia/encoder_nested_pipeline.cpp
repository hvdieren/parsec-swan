...
void nestedPipeline(struct thread_args *args, indep<list<chunk_t*>>chunks, int fd, int iteration, inoutdep<int>align)
{
	...
	for(iter = chunklist.begin(); iter != chunklist.end(); iter++)
	{
		...
		spawn(DeduplicateSwan, args, (inoutdep<chunk_t*>)chunk_obj, (inoutdep<int>) wait);
		spawn(CompressSwan, args, (inoutdep<chunk_t*>) chunk_obj);

		spawn(ReorderSwan, args, (indep<chunk_t*>)chunk_obj, fd, iteration, (inoutdep<int>) wait2);
	}
	ssync();
...
void SwanPipeline(struct thread_args *args)
{
	...
	*pr_buff_seek_ptr = pr_buff_seek;
	...
	list<chunk_t*>::iterator iter;
	object_t<chunk_t*>	chunk_obj, temp_obj;
	
	object_t<int*>		 	split_obj;
	object_t<size_t*>		bytes_left_obj, pr_buff_seek_obj;
	unversioned<u32int*>		rabintab_obj, rabinwintab_obj,
	...
	while(1)
	{
		...
		if(!(read_done) || iteration == 0)
		{
			...
			spawn(nestedPipeline, args, (indep<list<chunk_t*>>)chunks, fd, iteration, (inoutdep<int>)wait);
		}
		else
			break;
	}	
	ssync();
	stimer_tuck(&tmr, "Time for 1st stage");
	close(fd);
}
