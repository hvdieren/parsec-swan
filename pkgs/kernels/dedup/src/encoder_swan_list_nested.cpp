...
void sub_Compress_df(obj::inoutdep<chunk_t *>chunk) {
    // fprintf( stderr, "compress: chunk: %p (@%p)\n", (chunk_t*)chunk, chunk.get_version()->get_ptr() );
...
void sub_Deduplicate_df(obj::inoutdep<chunk_t *>chunk,
    ...
    // fprintf( stderr, "dedup: chunk: %p (@%p)\n", (chunk_t*)chunk, chunk.get_version()->get_ptr() );
...
void sub_SDC( chunk_t * chunk ) {
    // fprintf( stderr, "SDC: chunk: %p\n", (chunk_t*)chunk );
...
void sub_Wt( int fd_out,
	     obj::indep<std::list<chunk_t*> > list_in,
	     obj::inoutdep<void> token_B ) {

    std::list<chunk_t *> & list = list_in;
    // fprintf( stderr, "write: %d chunks in list %p\n", (int)list.size(), &list );
    for( auto I=list.begin(), E=list.end(); I != E; ++I ) {
      chunk_t * chunk = *I;
      assert(chunk!=NULL);
      // fprintf( stderr, "write: chunk: %p from list %p\n", (chunk_t*)chunk, &list );
...
void sub_FragmentRefine_df2( struct thread_args * args,
			     chunk_t * chunk,
			     obj::outdep<std::list<chunk_t *> > list_out ) {
  ...
  std::list<chunk_t *> & list = list_out;
...
void sub_Fragment_df2( struct thread_args * args ) {
  ...
  obj::object_t<std::list<chunk_t *> > list;
  ...
  while (1) {
    ...
    if(bytes_read == 0) {
      ...
      spawn( sub_FragmentRefine_df2, args, chunk, (obj::outdep<std::list<chunk_t*> >)list );
      spawn( sub_Wt, fd_out, (obj::indep<std::list<chunk_t*> >)list, (obj::inoutdep<void>)token_W );
