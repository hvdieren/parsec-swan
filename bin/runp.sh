...
for i in `seq $f $t` ; do
echo $cores $bench $i
./parsecmgmt -a run -p $bench -c $config -i native -n 8 -s "time numactl --physcpubind=+${CPUS[$cores]} --membind=0-$[(NUM_THREADS+7)/8-1]" > output_${bench}_${config}/${bench}_${config}_${cores}_${i}.txt
done
done
